...
DOMSerializer.prototype.processorSTYLE = function(node, callback) {
  ...
  this.tail = function(node, cssRules, callback) {        
    ...
    if(cssRule) {    
      // Grab the rule and check if the element has a property defined for it.
      var ruleText = $(node).css(cssRule.rule);
      if(ruleText) {        
        // TODO: Need to check we haven't got a dataurl already here.
        var src = ruleText.replace(/^url\(/,"").replace(/\)$/,"");     
        DOMSerializer._getImageData(src, {
            ok: function(dataURL) {
              $(node).css(cssRule.rule, "url("+dataURL+")");               
              dataURL = null;
              thisObj.tail(node, cssRules, callback);  // Tail
            },
            err: function(msg) {
          ...
          // Tail!
          this.tail(node, cssRules, callback);
        }
      } else {      
      callback();
    }
...
DOMSerializer.prototype.chunk = function(chunkSize, callback) {
  this.flatten(function(html) {    
    var idx      = 0;
    var chunkPos = 0;
    while(chunkPos < html.length) {
      callback.chunk(idx, html.substr(chunkPos,chunkSize))
      idx++;
      chunkPos = idx*chunkSize;
    }
